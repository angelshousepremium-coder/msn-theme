<?php
/**
 * Template Name: Фургоны и спецтехника на базе фургона
 * Template Post Type: page
 *
 * Путь: /wp-content/themes/msn/template-furgoni.php
 *
 * Используется для страниц:
 *   /avtofurgony
 *   /transportno-bytovye-mashiny-ural-tbm
 *   /peredvizhnye-masterskie-ural-parm-mp
 *   /avtomobili-dlya-perevozki-vzryvchatyh-veshhestv
 *   и других категорий связанных с фургонами
 *
 * Особенности шаблона:
 *   - Хлебные крошки и CTA-кнопки в одну строку
 *   - блок «Почему выбирают нас» хардкожен в stc-why-section (не зависит от Gutenberg)
 *   - Убран label "Официальный дилер" из topbar
 *   - CSS: инлайн переопределение .zh_btn / .main-product_btn1
 */
...
// Контент страницы — используем the_content() без parse_blocks
// (parse_blocks вызывает memory overflow на страницах с большим JSON от Editor Plus)
...
<!-- ── TOPBAR: хлебные крошки + кнопки в одну строку ── -->
...
<div class="stc-container stc-section" id="stc-products">
    ...
    <div class="stc-toolbar stc-animate">
        <div class="stc-toolbar__top">
            <div class="stc-toolbar__left">
                ...
                ?>
                <span class="stc-toolbar__count">Найдено: <strong><?php echo $n; ?> автомобил<?php echo $s; ?></strong></span>
                <?php endif; ?>
            </div>
    ...
    <?php if ($cat_slug && $have_products) : ?>
        ...
        <nav class="stc-pagination">
            <?php echo paginate_links([
                'base'      => str_replace($big, '%#%', esc_url(get_pagenum_link($big))),
                'format'    => '?paged=%#%', 'current' => $paged,
    ...
    <div class="stc-compare stc-animate">
        <div class="stc-compare__text">
            <h3 class="stc-compare__h">Нужен фургон под ваш проект?</h3>
            <p class="stc-compare__sub">Опишите задачу — подберём комплектацию, рассчитаем стоимость и подготовим КП в течение 15 минут.</p>
        </div>
    ...
    <div class="stc-seo stc-animate">
        <div class="stc-seo__label">О технике</div>
...
<div class="stc-why-section stc-section stc-animate">
    <div class="stc-container">
        ...
        <div class="stc-why-grid">
            <?php
            $why_items = [
                ['icon'=>'production','title'=>'Собственное производство','desc'=>'Полный цикл изготовления фургонов и надстроек. Выдача ПТС и ЭПТС.'],
                ['icon'=>'payment','title'=>'Гибкая форма оплаты','desc'=>'Покупка автотехники в лизинг. Простая процедура оформления сделки.'],
                ['icon'=>'design','title'=>'Собственное конструкторское бюро','desc'=>'Разработка нестандартных кузовов и спецнадстроек под задачи клиента.'],
                ['icon'=>'warranty','title'=>'Гарантия качества','desc'=>'2 года гарантии или 100 тыс. км пробега, гарантийное и сервисное обслуживание.'],
                ['icon'=>'timing','title'=>'Точное соблюдение сроков','desc'=>'Срок изготовления фургона — от 14 рабочих дней.'],
            ];
            $icons = [
                'production' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="32" height="32"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93A10 10 0 1 0 4.93 19.07"/><path d="M15.54 8.46A5 5 0 1 0 8.46 15.54"/></svg>',
                'payment'    => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="32" height="32"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>',
                'design'     => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="32" height="32"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',
                'warranty'   => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="32" height="32"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',
                'timing'     => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="32" height="32"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
            ];
    ...
    function stcObserve(){
        ...
        var io=new IntersectionObserver(function(entries){
            entries.forEach(function(entry){
                ...
                var el=entry.target;
                var d=el.classList.contains('stc-product-card')?(Array.from(el.parentNode.children).indexOf(el)%3)*80:0;
                setTimeout(function(){el.classList.add('stc-visible')},d);
                io.unobserve(el);
            });
...
(function(){
    ...
    /*
     * Берём только главную сетку hero.
     * Не трогаем вложенные .wp-block-columns внутри правой колонки,
     * например блок с Rutube-видео.
     */
    var mainCols = heroContent.querySelector(':scope > .wp-block-columns, :scope > .has-2-columns');
    if (!mainCols) {
        mainCols = heroContent.querySelector('.wp-block-columns, .has-2-columns');
    }
    if (!mainCols || !mainCols.children || mainCols.children.length < 2) return;

    var leftCol  = mainCols.children[0];
    var rightCol = mainCols.children[mainCols.children.length - 1];

    if (!leftCol || !rightCol) return;

    var benefits = document.querySelector('.stc-hero__benefits');
    var buttons  = document.querySelector('.stc-hero__buttons');
    ...
    /*
     * Подробнее ставим после последнего смыслового элемента текста:
     * p / ul / ol.
     * Пустые <p></p> от wpautop и блоки внутри Rutube игнорируем.
     */
    if (link) {
        var candidates = Array.prototype.slice.call(rightCol.children).filter(function(el) {
            var tag = el.tagName ? el.tagName.toLowerCase() : '';
            var text = (el.textContent || '').replace(/\s+/g, ' ').trim();

            if (['p', 'ul', 'ol'].indexOf(tag) === -1) return false;
            if (text.length < 10) return false;

            if (el.closest('.rutube-video-wrapper')) return false;
            if (el.querySelector('.rutube-video-wrapper')) return false;
            if (el.closest('.wp-block-columns .wp-block-columns')) return false;

            return true;
        });

        var target = candidates.length ? candidates[candidates.length - 1] : null;

        if (target && target.parentNode === rightCol) {
    ...
    /*
     * Кнопки должны идти после Подробнее.
     * Поэтому переносим их последними.
     */
    if (buttons) {
    ...
    if(btn&&body){btn.addEventListener('click',function(){
        ...
        body.classList.toggle('stc-seo__body--open',!o);
    ...
    /* Infinite scroll */
    (function(){
        var loading=false,noMore=false;
        ...
        function getNextUrl(){
            ...
            if(!next){var nxt=p2.querySelector('a.next');if(nxt)next=nxt.href}
            return next;
        }
        function loadMore(){
            if(loading||noMore||window.stcScrollBusy)return;
            var nextUrl=getNextUrl();if(!nextUrl){noMore=true;return}
