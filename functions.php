...
function final_theme_scripts() {
    ...
    wp_add_inline_script(
        'msn-js',
        'window.ajaxurl = window.ajaxurl || "' . esc_js( admin_url( 'admin-ajax.php' ) ) . '"; window.AJAX_URL = window.AJAX_URL || window.ajaxurl;',
        'before'
    );
...
// STC RESTORE 2026-07-01: внутренние страницы, не заменяет final_theme_styles()
add_action('wp_enqueue_scripts', function () {
    $theme_dir = get_template_directory();
    $theme_uri = get_template_directory_uri();
    $css_uri   = $theme_uri . '/css/';
    $js_uri    = $theme_uri . '/js/';
    ...
    $stc_old_page_templates = array(
        'template-contacts.php',
        'page-serv_center.php',
        'page-serv-center.php',
    );

    $stc_old_page_slugs = array(
        'contact',
        'serv_center',
        'serv-center',
    );

    $stc_modern_page_templates = array(
        'template-about.php',
        'template-achievements.php',
        'template-info.php',
        'template-lising.php',
        'template-mobilecenter.php',
    );

    $stc_modern_page_slugs = array(
        'o-kompanii',
        'nashi-dostizheniya',
        'informaczionnye-materialy',
        'kupit-ural-v-lizing',
        'mobilnyj-servisnyj-czentr',
        'mobilnyj-servisnyj-centr',
        'mobilnyy-servisnyy-tsentr',
    );

    if (
        is_page() &&
        (
            in_array($tpl, $stc_old_page_templates, true) ||
            in_array($tpl, $stc_modern_page_templates, true) ||
            is_page($stc_old_page_slugs) ||
            is_page($stc_modern_page_slugs)
        )
    ) {
        if ( ! wp_style_is('stc-page', 'enqueued') ) {
            wp_enqueue_style('stc-page', $css_uri . 'page.css', array('msn-style'), '20260701-page-restore');
        }

        if ( file_exists($css_dir . 'magnific-popup.css') && ! wp_style_is('magnific-popup', 'enqueued') ) {
            wp_enqueue_style('magnific-popup', $css_uri . 'magnific-popup.css', array('msn-style'), '1.1.0');
        }
    ...
    if ( is_page_template('template-about.php') || is_page('o-kompanii') ) {
        if ( file_exists($css_dir . 'about.css') && ! wp_style_is('stc-about', 'enqueued') ) {
            wp_enqueue_style('stc-about', $css_uri . 'about.css', array('msn-style', 'stc-page'), '20260701-about-restore');
        }
    }
    ...
    if ( is_page('remont-uralov') ) {
        if ( file_exists($css_dir . 'remont-uralov.css') && ! wp_style_is('stc-remont-uralov', 'enqueued') ) {
            wp_enqueue_style('stc-remont-uralov', $css_uri . 'remont-uralov.css', array('msn-style'), '20260701-remont-restore');
        }

        if ( file_exists($js_dir . 'remont-uralov.js') && ! wp_script_is('stc-remont-uralov', 'enqueued') ) {
            wp_enqueue_script('stc-remont-uralov', $js_uri . 'remont-uralov.js', array('jquery'), '20260701-remont-restore', true);
        }
    }
}, 99);
...
function final_theme_styles() {
    ...
    $tpl = get_page_template_slug(); // работает только для страниц (page)
    ...
    // template-product-v2.php — кастомный шаблон СТРАНИЦЫ (page, не product)
  ...
  wp_enqueue_style('stc-catalog-v2', $css . 'catalog-v2.css', ['msn-style'], '2.1');
    ...
    // Кастомные шаблоны страниц каталога v2
$catalog_v2_templates = [
    'template-product-v2.php',
    'template-cisterni.php',
    'template-furgoni.php',
];

if ( is_page_template( $catalog_v2_templates ) ) {
    $catalog_v2_path = get_theme_file_path( '/css/catalog-v2.css' );
    $catalog_v2_ver  = file_exists( $catalog_v2_path ) ? filemtime( $catalog_v2_path ) : '2.1';

    wp_enqueue_style(
        'stc-catalog-v2',
        $css . 'catalog-v2.css',
        ['msn-style', 'stc-header'],
        $catalog_v2_ver
    );
    ...
    // Страница "О компании" — отдельные scoped-стили, чтобы не трогать каталог и single product
    if ( $tpl === 'template-about.php' ) {
        wp_enqueue_style('stc-about', $css . 'about.css', ['msn-style', 'stc-header'], '20260701-about-hero-certs');
        wp_enqueue_style('magnific-popup', $css . 'magnific-popup.css', [], '1.1.0');
    }

    // Страницы товара WooCommerce (is_product)
    if ( is_product() ) {
        wp_enqueue_style('stc-product',    $css . 'product.css',    ['msn-style'], '2.1');
        wp_enqueue_style('stc-catalog',    $css . 'catalog.css',    ['msn-style'], '2.1');
        wp_enqueue_style('stc-sticky-cta', $css . 'sticky-cta.css', ['msn-style'], '2.1');
	...
	// Глобальные стили Magnific Popup / CF7-попапов из шапки и карточек
    wp_enqueue_style(
        'magnific-popup',
        $css . 'magnific-popup.css',
        ['msn-style'],
    ...
    wp_enqueue_style(
        'stc-popup',
        $css . 'popup.css',
        ['msn-style', 'magnific-popup'],
        '20260701-popup-v1'
    );
	// Внутренние страницы, включая лизинг
if ( is_page() && ! is_front_page() && ! is_product() ) {
    wp_enqueue_style(
        'stc-page',
        $css . 'page.css',
        ['msn-style', 'stc-header'],
...
// Кастомная карточка woocommerce/content-product.php сама управляет ссылкой.
// Убираем стандартное открытие/закрытие <a>, чтобы не было вложенных ссылок.
...
function change_requerst_vars_for_product_cat($vars) {
    ...
    // Без /product-category/ категория с таким же slug может конфликтовать со страницей,
    // записью или товаром. В таком случае не перехватываем запрос под product_cat.
    $conflict_post_id = $wpdb->get_var(
        $wpdb->prepare(
            "SELECT ID
             FROM $wpdb->posts
             WHERE post_name = %s
...
<div class="col-lg-6">
<?php
$tabs3img  = get_field('tabs3');
$tabs31imgs = get_field('tabs31');
if ( $tabs3img || $tabs31imgs ) : ?>
<ul class="zoom-gallery kabinagallery">
  <?php if ( $tabs3img ) : ?>
  <li class="kabinagallery__main">
    <a href="<?php echo esc_url($tabs3img['url']); ?>">
      <img src="<?php echo esc_url($tabs3img['url']); ?>" alt="<?php echo esc_attr($tabs3img['alt']); ?>" />
    </a>
  </li>
  <?php endif; ?>
  <?php if ( $tabs31imgs ) : foreach ( $tabs31imgs as $timg ) : ?>
  <li>
    <a href="<?php echo esc_url($timg['url']); ?>">
      <img src="<?php echo esc_url($timg['sizes']['thumbnail']); ?>" alt="<?php echo esc_attr($timg['alt']); ?>" />
    </a>
  </li>
  <?php endforeach; endif; ?>
</ul>
...
add_action('wp_enqueue_scripts', function () {
    ...
    $old_handles = array(
        'stc-info-load-more',
        'stc-info-load-more-v2',
        'stc-info-load-more-v3',
        'stc-info-load-more-v4',
        'info-load-more',
        'info-load-more-v2',
        'info-load-more-v3',
        'info-load-more-v4',
